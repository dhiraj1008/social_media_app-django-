    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                profile_obj = Profile.objects.create(user=user_model,id_user=user_model.id)
                profile_obj.save()
...
def follow(request):
  if request.method == "POST":
      ...
      # need to check is that current user is folowwing this profile he visisted or not
      #unfollow part
      if FollowerCount.objects.filter(follower=follower,user=user):
          delete_follower = FollowerCount.objects.filter(follower=follower,user=user)
